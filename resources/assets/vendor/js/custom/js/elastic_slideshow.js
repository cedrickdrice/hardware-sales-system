...
;(function(window) {
	...
	function CircleSlideshow(el, options) {
		...
		// items
		this.items = [].slice.call(this.el.querySelectorAll('.e-slide'));
		// total items
	...
	CircleSlideshow.prototype._init = function() {
		...
		this.items.forEach(function(item) {
			...
			var slideEl = item.querySelector('.e-slide__item');
			slideEl.insertBefore(expanderEl, slideEl.firstChild);
		});

		// position current item:
		classie.add(this.items[this.current], 'e-slide--current');
		// event binding
		this._initEvents();
	};
	...
	CircleSlideshow.prototype._navigate = function(dir) {
		...
		var self = this,
			...
			currentEl = itemCurrent.querySelector('.e-slide__item'),
			currentTitleEl = itemCurrent.querySelector('.e-slide__title');
		...
		var itemNext = this.items[this.current],
			nextEl = itemNext.querySelector('.e-slide__item'),
			nextTitleEl = itemNext.querySelector('.e-slide__title');
		...
		dynamics.animate(nextEl, 
			...
			}, 
			{
				...
				complete: function() {
					self.items.forEach(function(item) { classie.remove(item, 'e-slide--current'); });
					classie.add(itemNext, 'e-slide--current');
				}
			}
		);
	...
	CircleSlideshow.prototype._openContent = function(item) {
		...
		var self = this,
			...
			smallImgEl = item.querySelector('.e-slide__img--small'),
			contentEl = item.querySelector('.e-slide__content'),
			largeImgEl = contentEl.querySelector('.e-slide__img--large'),
			titleEl = contentEl.querySelector('.e-slide__title--main'),
			descriptionEl = contentEl.querySelector('.e-slide__description'),
			priceEl = contentEl.querySelector('.e-slide__price'),
			buyEl = contentEl.querySelector('.button--buy');

		// add slide--open class to the item
		classie.add(item, 'e-slide--open');
		// prevent scrolling
	...
	CircleSlideshow.prototype._closeContent = function() {
		...
		classie.add(item, 'e-slide--close');
		...
		dynamics.animate(largeImgEl, 
			...
			}, 
			{
				...
				complete: function() {
					...
					classie.remove(item, 'e-slide--open');
					// remove slide--close class to the item
					classie.remove(item, 'e-slide--close');
					// allow scrolling
					classie.remove(bodyEl, 'lockscroll');
